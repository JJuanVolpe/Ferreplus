...
from .forms import RecoveryForm, crear_intercambio_con_espera_de_ofertas, ProductForm, RatingForm
...
def miPerfil(request):
    ...
    if Rating.objects.filter(profile=usuario.profile).exists(): 
        average_rating = Rating.objects.filter(profile=usuario.profile).get()
        puntuacion=average_rating.rating / average_rating.cantValoraciones
